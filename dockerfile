...
# RUN pip install --no-cache-dir tenacity==9.1.2
...
RUN --mount=type=cache,target=${UV_CACHE_DIR} \
    uv sync --verbose --no-install-project --override-default-index-url
...
# Cache buster: 1 - verbose flag added
RUN --mount=type=cache,target=${UV_CACHE_DIR} \
    uv sync --verbose --no-editable --override-default-index-url
